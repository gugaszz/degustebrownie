      ...
      <div className="bg-white rounded-3xl max-w-lg w-full shadow-2xl border border-[#EAE5E2] overflow-hidden">
        <div className="px-5 py-4 border-b border-[#EAE5E2] flex items-center justify-between bg-[#F8F6F4]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-[#201A17]">Novo Pedido de Compra</h3>
              <p className="text-[11px] text-[#746A65]">Reposição de estoque com fornecedor</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-[#201A17] mb-1">Fornecedor:</label>
              <select
                value={supplierId}
          ...
          <div>
            <label className="block text-xs font-bold text-[#201A17] mb-2">Quantidades por Sabor:</label>
                  ...
                  <div key={f.id} className="p-3 rounded-xl border border-[#EAE5E2] flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="w-16 text-center text-xs font-bold bg-[#F8F6F4] border border-[#EAE5E2] rounded-lg py-1"
                      />
                      <button
                        type="button"
                        onClick={() => handleQtyChange(f.id, qty + 10)}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-[#201A17] mb-1">Data Prevista de Entrega:</label>
              <input
                type="date"
                value={expectedDate}
          ...
          <div className="pt-2 border-t border-[#EAE5E2] flex items-center justify-between">
            <div>
              <span className="text-xs text-[#746A65] block">{totalUnits} brownies total</span>
              <span className="text-lg font-black text-[#3B241C]">{formatCurrency(totalAmount)}</span>
            </div>

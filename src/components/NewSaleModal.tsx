...
export const NewSaleModal: React.FC<NewSaleModalProps> = ({ isOpen, onClose, onSaleComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl max-w-lg w-full shadow-2xl border border-[#E7E5E2] overflow-hidden flex flex-col max-h-[92vh]">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#6B6B6B]">
                Vendedor: <span className="font-semibold text-[#141414]">{currentSeller.name}</span>
              </p>
            </div>
          </div>
          <button
        ...
        {!completedSale && (
          <div className="bg-white px-5 pt-3 pb-1 border-b border-[#E7E5E2]">
            <div className="flex items-center justify-between text-[11px] font-semibold text-[#6B6B6B] mb-1.5">
            ...
            <div className="h-1.5 w-full bg-[#EDEBE8] rounded-full overflow-hidden">
              <div
                className="h-full bg-[#141414] transition-all duration-300 rounded-full"
        ...
        <div className="p-5 overflow-y-auto flex-1">
          ...
          {step === 1 && !completedSale && (
            <div className="space-y-5">
              ...
              {state.currentUser.role === 'owner' && (
                <div className="p-3 bg-[#F3F1EE] rounded-2xl border border-[#E7E5E2] space-y-1.5">
                  <label className="block text-[11px] font-bold text-[#6B6B6B] uppercase tracking-wider">
                    Origem da Venda:
                  </label>
                  <select
                    ...
                    className="w-full text-xs font-bold bg-white border border-[#E7E5E2] rounded-xl px-3 py-2 text-[#111111] outline-hidden"
                  >
              ...
              <div className="text-center">
                <h3 className="text-lg font-bold text-[#111111]">Quantos brownies?</h3>
                <p className="text-xs text-[#6B6B6B] mt-0.5">
                  Estoque disponível na origem: <strong className="text-[#111111]">{totalSellerStock} unidades</strong>
                </p>
              </div>
              ...
              <div className="flex items-center justify-center gap-5 py-4">
                <button
                  ...
                  className="w-14 h-14 rounded-2xl border border-[#E7E5E2] bg-[#F6F5F3] text-[#141414] hover:bg-[#EDEBE8] disabled:opacity-40 disabled:pointer-events-none flex items-center justify-center transition shadow-xs"
                >
                  <Minus className="w-6 h-6" />
                </button>

                <div className="w-24 text-center">
                  <span className="text-5xl font-extrabold text-[#141414] tabular-nums tracking-tight">
                    {totalQuantity}
                  </span>
                  <span className="block text-xs font-semibold text-[#6B6B6B] mt-1">
                ...
                <button
                  ...
                  className="w-14 h-14 rounded-2xl bg-[#141414] text-white hover:bg-[#0A0A0A] disabled:opacity-40 disabled:pointer-events-none flex items-center justify-center transition shadow-md"
                >
              ...
              {/* Quick Options */}
              <div>
                <span className="block text-[11px] font-semibold text-[#6B6B6B] uppercase tracking-wider text-center mb-2">
                  Atalhos Rápidos
                </span>
                    ...
                    <button
                      ...
                      className={`py-2 rounded-xl text-sm font-bold border transition ${
                        totalQuantity === q
                          ...
                          : 'bg-white text-[#111111] border-[#E7E5E2] hover:bg-[#F6F5F3]'
              ...
              <div className="p-4 rounded-2xl bg-[#F6F5F3] border border-[#E7E5E2] flex items-center justify-between">
                <div>
                  <span className="text-xs text-[#6B6B6B]">Regra de preço aplicada:</span>
                  ...
                  {applyDiscount && totalQuantity >= 2 && (
                    <span className="text-[11px] font-semibold text-[#1B8A4F] flex items-center gap-1 mt-0.5">
                      <Sparkles className="w-3 h-3" /> Economia de {formatCurrency(totalQuantity * 1.0)} para o cliente
                    </span>
                  )}
                </div>
                <div className="text-right">
                  <span className="text-xs text-[#6B6B6B]">Total a pagar:</span>
                  <div className="text-2xl font-black text-[#141414] tabular-nums">
              ...
              <button
                ...
                className={`w-full p-3.5 rounded-2xl border flex items-center justify-between transition text-left ${
                  !discountEligible
                    ? 'border-[#E7E5E2] bg-gray-50 opacity-50 cursor-not-allowed'
                    : applyDiscount
                    ? 'border-[#1B8A4F] bg-green-50'
                    : 'border-[#E7E5E2] bg-white hover:bg-[#F6F5F3]'
                }`}
              >
                <div>
                ...
                <div
                  className={`w-10 h-6 rounded-full flex items-center px-0.5 shrink-0 transition ${
                    applyDiscount && discountEligible ? 'bg-[#1B8A4F] justify-end' : 'bg-[#DFDDD9] justify-start'
                  }`}
                >
              ...
              <button
                ...
                className="w-full py-3.5 px-4 rounded-2xl bg-[#141414] text-white font-bold text-sm hover:bg-[#0A0A0A] transition flex items-center justify-center gap-2 shadow-md disabled:opacity-40"
              >
          ...
          {step === 2 && !completedSale && (
            <div className="space-y-4">
              <div>
                <h3 className="text-base font-bold text-[#111111]">Escolha os sabores</h3>
                <p className="text-xs text-[#6B6B6B]">
                  Distribua exatamente os <strong className="text-[#141414]">{totalQuantity} brownies</strong> entre os sabores disponíveis em seu estoque:
                </p>
              </div>
              ...
              <div className={`p-3 rounded-xl text-xs font-semibold flex items-center justify-between ${
                remainingToSelect === 0
                  ? 'bg-green-50 text-[#1B8A4F] border border-green-200'
                  : 'bg-amber-50 text-[#A9761F] border border-amber-200'
              }`}>
                  ...
                  return (
                    <div
                      ...
                      className={`p-3.5 rounded-2xl border transition flex items-center justify-between ${
                        selected > 0
                          ? 'border-[#141414] bg-[#F6F5F3]'
                          : isOutOfStock
                          ? 'border-[#E7E5E2] opacity-40 bg-gray-50'
                          : 'border-[#E7E5E2] bg-white hover:border-[#6B6B6B]'
                      }`}
                    >
                      <div>
                        ...
                        <span className="text-xs text-[#6B6B6B]">
                          Estoque disponível: <strong className={stock < 3 ? 'text-[#B3403D]' : 'text-[#111111]'}>{stock} un.</strong>
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          disabled={selected <= 0}
                          className="w-9 h-9 rounded-xl border border-[#E7E5E2] bg-white text-[#111111] hover:bg-[#EDEBE8] disabled:opacity-30 disabled:pointer-events-none flex items-center justify-center transition"
                        >
                          <Minus className="w-3.5 h-3.5" />
                        </button>
                        <span className="w-8 text-center font-bold text-base text-[#141414] tabular-nums">
                          {selected}
                        </span>
                        <button
                          onClick={() => handleFlavorCount(flavor.id, 1)}
                          disabled={isOutOfStock || selected >= stock || selectedFlavorsCount >= totalQuantity}
                          className="w-9 h-9 rounded-xl bg-[#141414] text-white hover:bg-[#0A0A0A] disabled:opacity-30 disabled:pointer-events-none flex items-center justify-center transition"
                        >
              ...
              <div className="flex gap-2 pt-2">
                <button
                  onClick={() => setStep(1)}
                  className="py-3 px-4 rounded-2xl border border-[#E7E5E2] text-xs font-semibold text-[#6B6B6B] hover:bg-[#F6F5F3] transition flex items-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="flex-1 py-3 px-4 rounded-2xl bg-[#141414] text-white text-xs font-bold hover:bg-[#0A0A0A] transition disabled:opacity-40 flex items-center justify-center gap-2 shadow-md"
                >
          ...
          {step === 3 && !completedSale && (
            <div className="space-y-4">
              <div>
                <h3 className="text-base font-bold text-[#111111]">Resumo da venda</h3>
                <p className="text-xs text-[#6B6B6B]">
                  Confira as informações antes de apresentar o Pix para o cliente:
                </p>
              </div>

              <div className="p-4 rounded-2xl bg-[#F6F5F3] border border-[#E7E5E2] space-y-3">
                <div className="flex justify-between items-center text-xs pb-2 border-b border-[#E7E5E2]">
                  <span className="text-[#6B6B6B]">Total de Brownies:</span>
                  <span className="font-bold text-[#111111]">{totalQuantity} unidades</span>
                </div>

                <div className="space-y-1.5 pb-2 border-b border-[#E7E5E2]">
                  ...
                  {Object.entries(flavorSelections).map(([flavorId, qty]) => {
                    ...
                    return (
                      <div key={flavorId} className="flex justify-between text-xs">
                        <span className="text-[#111111] font-medium">{flv?.name}</span>
                        <span className="font-bold text-[#141414] tabular-nums">{qty}x {formatCurrency(unitPrice)}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="flex justify-between items-center text-xs pb-1">
                  <span className="text-[#6B6B6B]">Preço unitário aplicado:</span>
                  <span className="font-semibold text-[#111111]">
                    {formatCurrency(unitPrice)} cada
                    {discountEligible && !applyDiscount && (
                      <span className="text-[10px] text-[#A9761F] font-bold ml-1">(sem desconto)</span>
                    )}
                  </span>
                </div>

                <div className="flex justify-between items-baseline pt-2 border-t border-[#E7E5E2]">
                  <span className="font-bold text-sm text-[#111111]">Valor Total:</span>
              ...
              {/* Notice */}
              <div className="p-3 rounded-xl bg-[#EDEBE8] text-[#141414] text-xs flex items-start gap-2">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-3 px-4 rounded-2xl bg-[#141414] text-white text-xs font-bold hover:bg-[#0A0A0A] transition flex items-center justify-center gap-2 shadow-md"
                >
          ...
          {step === 4 && !completedSale && (
            <div className="space-y-4 text-center">
              <div>
                <h3 className="text-base font-bold text-[#111111]">Pagamento via Pix</h3>
                <p className="text-xs text-[#6B6B6B]">
              ...
              <div className="p-4 bg-white border border-[#E7E5E2] rounded-3xl shadow-sm inline-block mx-auto">
                <img
                ...
                <div className="mt-2 text-center">
                  <span className="text-xs text-[#6B6B6B] block">Cliente deve digitar este valor no banco:</span>
                  <span className="text-2xl font-black text-[#141414] tabular-nums">
                    {formatCurrency(totalAmount)}
                  </span>
                </div>
              ...
              <div className="space-y-1.5 text-left">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 text-xs bg-[#F6F5F3] border border-[#E7E5E2] rounded-xl px-3 py-2 text-[#6B6B6B] font-mono select-all truncate"
                  />
                  <button
                    onClick={handleCopyPix}
                    className="px-3 py-2 bg-[#EDEBE8] text-[#141414] font-semibold text-xs rounded-xl hover:bg-[#141414] hover:text-white transition flex items-center gap-1.5 shrink-0"
                  >
              ...
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-left text-xs text-[#A9761F] flex items-start gap-2">
              ...
              <div className="flex gap-2 pt-2">
                <button
                  onClick={() => setStep(3)}
                  className="py-3 px-4 rounded-2xl border border-[#E7E5E2] text-xs font-semibold text-[#6B6B6B] hover:bg-[#F6F5F3] transition"
                >
                  Voltar
                </button>
                <button
                  ...
                  className="flex-1 py-3 px-4 rounded-2xl bg-[#1B8A4F] text-white text-xs font-bold hover:bg-[#145C36] transition flex items-center justify-center gap-2 shadow-md"
                >
          ...
          {completedSale && (
            <div className="py-4 text-center space-y-4">
              <div className="w-16 h-16 rounded-full bg-green-100 text-[#1B8A4F] flex items-center justify-center mx-auto shadow-inner">
                <CheckCircle2 className="w-9 h-9" />
              </div>

              <div>
                <h3 className="text-xl font-extrabold text-[#111111]">Venda Confirmada!</h3>
              ...
              {/* Sales Metrics Card */}
              <div className="p-4 rounded-2xl bg-[#F6F5F3] border border-[#E7E5E2] space-y-3 text-left">
                <div className="flex justify-between items-center text-xs pb-2 border-b border-[#E7E5E2]">
                  <span className="text-[#6B6B6B]">Código da Transação:</span>
                  <span className="font-mono font-bold text-[#141414]">{completedSale.pix_txid}</span>
                </div>
                <div className="flex justify-between items-center text-xs pb-2 border-b border-[#E7E5E2]">
                  <span className="text-[#6B6B6B]">Quantidade:</span>
                  <span className="font-bold text-[#111111]">{completedSale.total_quantity} brownies</span>
                </div>
                <div className="flex justify-between items-center text-xs pb-2 border-b border-[#E7E5E2]">
                  <span className="text-[#6B6B6B]">Valor Recebido via Pix:</span>
                  <span className="font-black text-base text-[#111111] tabular-nums">
                    {formatCurrency(completedSale.total_amount)}
                  </span>
                </div>
                <div className="flex justify-between items-center text-xs pt-1">
                  <span className="text-[#6B6B6B] font-semibold">Sua Comissão Gerada:</span>
                  <span className="font-black text-base text-[#1B8A4F] tabular-nums bg-green-50 px-2 py-0.5 rounded-lg border border-green-200">
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="flex-1 py-3 px-4 rounded-2xl bg-[#141414] text-white text-xs font-bold hover:bg-[#0A0A0A] transition flex items-center justify-center gap-1.5 shadow-md"
                >
      ...
      {showConfirmDialog && (
        <div className="fixed inset-0 z-60 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-[#E7E5E2] text-center space-y-4">
            <div className="w-12 h-12 rounded-2xl bg-amber-100 text-[#A9761F] flex items-center justify-center mx-auto">
              <ShieldAlert className="w-6 h-6" />
            </div>

            <div>
              <h4 className="text-base font-bold text-[#111111]">Pagamento recebido?</h4>
              <p className="text-xs text-[#6B6B6B] mt-1 leading-relaxed">
                Confirme apenas depois de verificar o comprovante do Pix com o cliente. O estoque será deduzido imediatamente.
              </p>
            </div>

            <div className="p-3 bg-[#F6F5F3] rounded-xl text-xs flex justify-between font-semibold">
              <span className="text-[#6B6B6B]">Valor confirmado:</span>
              <span className="text-[#141414] font-bold">{formatCurrency(totalAmount)}</span>
            </div>
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                disabled={isProcessing}
                className="flex-1 py-2.5 px-3 rounded-xl border border-[#E7E5E2] text-xs font-semibold text-[#6B6B6B] hover:bg-[#F6F5F3]"
              >
                Voltar
              </button>
              <button
                ...
                disabled={isProcessing}
                className="flex-1 py-2.5 px-3 rounded-xl bg-[#1B8A4F] text-white text-xs font-bold hover:bg-[#145C36] shadow-sm flex items-center justify-center gap-1.5"
              >

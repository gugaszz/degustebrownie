    ...
    <div className="space-y-5 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-4 rounded-3xl bg-white border border-[#E7E5E2] shadow-xs">
          <span className="text-xs text-[#6B6B6B] font-medium block">Total Comercializado</span>
          <div className="text-2xl font-black text-[#111111] tabular-nums mt-1">
            {formatCurrency(totalAmount)}
          </div>
          <span className="text-[11px] text-[#6B6B6B] mt-0.5 block">{mySales.length} transações</span>
        </div>
        <div className="p-4 rounded-3xl bg-white border border-[#E7E5E2] shadow-xs">
          <span className="text-xs text-[#6B6B6B] font-medium block">Total de Comissões Ganhas</span>
      ...
      {/* Filter / Search */}
      <div className="p-3 bg-white rounded-2xl border border-[#E7E5E2] shadow-xs">
        <input
          type="text"
          placeholder="Filtrar por código Pix TXID ou sabor..."
          value={searchTerm}
          onChange={e => setSearchTerm(e.target.value)}
          className="w-full text-xs px-3 py-2 bg-[#F6F5F3] border border-[#E7E5E2] rounded-xl text-[#111111] focus:outline-none"
        />
      </div>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[#E7E5E2]/70">
              {filteredSales.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-[#6B6B6B]">
